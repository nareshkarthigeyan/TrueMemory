...
app = modal.App("longmemeval-truememory-pro")
...
@app.function(image=img, secrets=[modal.Secret.from_name("openrouter-key")],
              timeout=14400, memory=8192, gpu="A100-80GB")
...
def orchestrate(dataset: list, run_id: int = 1, smoke: bool = False):
    system = "truememory_pro_longmemeval"
    ...
    result = {
        "system": "truememory_pro", "benchmark": "LongMemEval_oracle",
...
def main(smoke: bool = False, run_id: int = 1, dataset: str = None):
    if dataset is None:
        for c in [Path(__file__).parent / "data" / "longmemeval_oracle.json"]:
            if c.exists(): dataset = str(c); break
    if not dataset:
        print("ERROR: longmemeval_oracle.json not found"); sys.exit(1)
